...
type Auto = {
  Marke_automobila: string;
  Model_automobila: string;
...
//prvi zadatak
const mapaOsoba = osobe.reduce((acc: any, osoba) => {
  const { id, ime, prezime, brojKartice } = osoba;
  //@ts-ignore

  const mapAutomobila = automobili
    .filter((automobil) => id === automobil.idVlasnika)
    .map((automobil) => {
      return {
  ...
  acc[ime] = { id, prezime, brojKartice, mapAutomobila };
  return acc;
}, {});
...
// drugi zadatak
const najstarijiAutomobil = automobili.reduce((acc, automobil) => {
...
const personWithOldestCar = osobe.filter((osoba) => {
...
const rezultatTreceg = osobe.reduce(
  (acc: any, osoba) => {
    const { id, ime, prezime, brojKartice } = osoba;
    const mapaAutomobila: string[] = automobili
      .filter((automobil) => id === automobil.idVlasnika)
      .map((automobil) => {
...
const rezultatCetvrtog = osobe.map((osoba) => {
  let sumaSvihAutomobila = automobili
    .filter((automobil) => osoba.id === automobil.idVlasnika)
    .reduce((acc, automobil) => {
      acc += automobil.cena;
      return acc;
    }, 0);
...
const rezultatPetog = rezultatCetvrtog.filter(
  (osoba) => osoba.CenaVoznogParka > 3000
);
