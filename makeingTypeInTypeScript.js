...
var arrayOfCards = account.transactions.map(function (transaction) { return transaction.card; });
//console.log(arrayOfCards);
var arrayOfDaysInFuture = arrayOfCards.filter(function (card) {
    var today = new Date();
    return new Date(card.expiry).getTime() > today.getTime();
});
console.log(arrayOfDaysInFuture);
function minimum(arrayOfDaysInFuture) {
    var min = new Date(arrayOfDaysInFuture[0].expiry).getTime();
    //console.log(min);
    var minCard = arrayOfDaysInFuture[0];
    //console.log(minCard);
    for (var i = 1; i < arrayOfDaysInFuture.length; i++) {
        var currentDateTime = new Date(arrayOfDaysInFuture[i].expiry).getTime();
